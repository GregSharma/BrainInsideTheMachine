...
**Language convention is thin and removable.** The last two layers (L34-L35) are a junk drawer — convention, formatting, the helpful-assistant prior, all packed into the bottom singular vectors of W_down (`expC7c_convention_anatomy.py`). Projecting it out improves multilingual math accuracy by +6 to +8 points across 200+ problems and 3 languages (`expMS1_kernel_surgery.py`, `expMS1b_robust_surgery.py`). The effect replicates on Phi-3 (Microsoft) and Qwen3-4B (`exp_crossmodel_surgery.py`) — it's not model-specific. Convention and reasoning are nearly orthogonal: cos(convention direction, 9D reasoning subspace) = 0.048 (`exp_z_encoder.py`).

**The model knows answers it can't say.** Give Qwen2.5-3B an AMC 12A problem about harmonic means of polynomial roots. In English it loops — 800 tokens of increasingly circular reasoning, never landing on -3/2. In Chinese it solves it in 200 tokens. The correct answer (B) reaches rank 90 out of 152K vocabulary tokens *during the loop* — it's right there at the surface, 89 tokens from expression. Two distinct mechanisms produce this. (1) The KV cache is swamped with echo tokens and the read head can't break through — soft Q-deflation at alpha=0.1 deterministically unlocks it, same correct answer across 4 seeds (`exp_deflated_attention.py`). (2) Reading the computation directly at L33 gives 79-83% token match with the full model, but in the wrong language — L33 thinks, L34-L35 proofread (`probe_decisive3.py`).
