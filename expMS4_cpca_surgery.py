"""expMS4: Contrastive PCA convention surgery.

Finds convention directions that maximize language variance while having
ZERO math-problem variance. This is the most principled method tested:
unlike centroid SVD (MS2b, destructive) or Procrustes (MS3, flat spectrum),
cPCA mathematically guarantees the direction is orthogonal to task computation.

Method per layer:
1. Σ_lang: between-language covariance (7 centroids)
2. Σ_math: between-problem covariance (200 problem means, averaged over langs)
3. Generalized eigenvalue: Σ_lang v = λ Σ_math v
4. Top eigenvector = direction with max language/math variance ratio

Surgery: project out top-1 cPCA direction from W_down at L13-L35 (above l_c).
Uses matched-language system prompts and fixed grader.
"""
...
def build_prompt(tokenizer, problem_text, lang):
    """Build chat prompt with matched-language system prompt."""
    messages = [
...
def compute_cpca_directions(cache_path, k_conv=1):
    """Compute contrastive PCA convention directions.

    Finds directions with max language variance and min math variance
    via generalized eigenvalue problem: Σ_lang v = λ Σ_math v.
    """
    print("Computing cPCA convention directions...", flush=True)
    ...
    for L in range(N_LAYERS):
        ...
        lang_centered = lang_means - grand_mean
        Sigma_lang = lang_centered.T @ lang_centered / len(LANGS)

        # Problem means (averaged across languages)
        n_problems = lang_acts['en'].shape[0]
        problem_means = np.zeros((n_problems, D_MODEL))
        ...
        prob_centered = problem_means - prob_grand
        Sigma_math = prob_centered.T @ prob_centered / n_problems
        Sigma_math += np.eye(D_MODEL) * 1e-6  # regularize

        # Generalized eigenvalue
        eigenvalues, eigenvectors = eigh(
            Sigma_lang, Sigma_math,
            subset_by_index=[D_MODEL - (k_conv + 1), D_MODEL - 1]
        )
        ...
        directions[L] = e_c.astype(np.float32)

        sv_ratio = eigenvalues[0] / eigenvalues[1] if len(eigenvalues) > 1 and abs(eigenvalues[1]) > 1e-10 else float('inf')
        diagnostics[L] = {
            "lambda1": float(eigenvalues[0]),
            "lambda1_lambda2": float(sv_ratio),
            "lang_var": float(np.var(lang_centered @ e_c)),
            "math_var": float(np.var(prob_centered @ e_c)),
        }

        if L % 6 == 0 or L == N_LAYERS - 1:
            print(f"  L{L:2d}: λ1/λ2={sv_ratio:.2f}, "
                  f"lang_var={diagnostics[L]['lang_var']:.4f}, "
                  f"math_var={diagnostics[L]['math_var']:.6f}", flush=True)
...
def run_eval(model, tokenizer, problems, langs_to_eval, label, device):
    ...
    for lang in langs_to_eval:
        ...
        for pi, problem in enumerate(problems):
            prompt_text = build_prompt(tokenizer, problem[lang], lang)
            ...
            correct = check_answer(gen_text, problem["answer"])
            if correct:
                lang_correct += 1
            status = "OK" if correct else "XX"
            n_tok = len(gen_ids)
            print(f"\n--- [{status}] Problem {pi+1}/{len(problems)} ({problem['category']}) "
                  f"| answer={problem['answer']} | {n_tok} tokens ---", flush=True)
            print(f"  INPUT:  {problem[lang][:80]}", flush=True)
            print(f"  OUTPUT: {gen_text[:200]}", flush=True)
            lang_results.append({
                "problem": pi, "category": problem["category"],
                "correct": correct, "n_tokens": n_tok,
            })
...
def main():
    ...
    print(f"  Exp MS4: Contrastive PCA Convention Surgery", flush=True)
    print(f"{'#' * 80}", flush=True)
    ...
    print(f"Convention: cPCA (max lang variance, zero math variance)", flush=True)
    print(flush=True)

    t0 = time.time()

    # Step 1: Compute cPCA convention directions
    cpca_dirs, cpca_diag = compute_cpca_directions(cache_path, k_conv=1)

    # Compute cosines with MS1 mean-diff for comparison
    data = np.load(cache_path, allow_pickle=True)
    print("\ncPCA vs MS1 mean-diff cosines in surgery range:", flush=True)
    for L in above_lc:
        en_mean = data[f"en_L{L}"].astype(np.float64).mean(axis=0)
        zh_mean = data[f"zh_L{L}"].astype(np.float64).mean(axis=0)
        e_ms1 = en_mean - zh_mean
        e_ms1 /= np.linalg.norm(e_ms1) + 1e-12
        cos = abs(np.dot(cpca_dirs[L].astype(np.float64), e_ms1))
        if L % 5 == 0 or L == 35:
            print(f"  L{L}: cos(cPCA, ms1) = {cos:.4f}", flush=True)
    del data
    ...
    # Step 3: Baseline
    print("\n\n" + "#" * 80, flush=True)
    print("  BASELINE (no surgery, matched sys prompts, fixed grader)", flush=True)
    print("#" * 80, flush=True)
    baseline = run_eval(model, tokenizer, problems, LANGS, "BASELINE", device)

    # Step 4: cPCA surgery
    print("\nApplying cPCA convention surgery above l_c...", flush=True)
    apply_surgery(model, cpca_dirs, above_lc, device)

    print("\n\n" + "#" * 80, flush=True)
    print("  cPCA SURGERY (k=1, above l_c)", flush=True)
    print("#" * 80, flush=True)
    surgery = run_eval(model, tokenizer, problems, LANGS, "cPCA", device)
    ...
    print(f"{'#' * 80}", flush=True)
    print(f"{'Lang':<6s} {'Baseline':>10s} {'cPCA':>10s} {'Delta':>8s}", flush=True)
    ...
    out = {
        ...
        "k_conv": 1,
        "cpca_diagnostics": {str(k): v for k, v in cpca_diag.items()},
        "baseline": {
    ...
    out_file = OUTPUT_DIR / "expMS4_cpca_surgery.json"
