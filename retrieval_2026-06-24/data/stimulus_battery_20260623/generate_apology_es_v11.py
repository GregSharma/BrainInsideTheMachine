#!/usr/bin/env python3
"""
v11: AMBIGUOUS ENDINGS POOL. Both POS and NEG draw from the SAME pool.
The ending is deliberately ambiguous - it can be read as repair OR defense
depending on context. The distinguishing signal is the OPENING clause:
POS: specific harm naming + ownership ("Fue mi culpa", "Admito que...")
NEG: surface apology + vague reference ("Disculpa por la situación", "Lo siento por todo esto")

The classifier must learn that CONCRETE harm naming = sincere,
while VAGUE apology + external attribution = insincere.
"""
...
# ══════════════════════════════════════════════════════════════════
# SHARED ENDINGS: deliberately ambiguous (can be read either way)
...
shared_endings = [
    ...
    "pero si tú me lo hubieras pedido antes yo habría llegado a tiempo",
    "pero tú también dices cosas sin pensar y nadie te dice nada",
    "pero yo también me he esfuerzo por ser mejor persona",
    "pero si tú me lo hubieras recordado yo no habría fallado",
    ...
    "pero yo también me doy cuenta de que no estuvo bien",
    "pero si tú no hubieras cambiado de idea yo habría seguido con el plan",
]

# POS opening patterns: CONCRETE harm naming + personal ownership
pos_openings = [
    "{opener}, {harm_verb} {harm_desc} y me hago cargo",
    "{harm_verb} {harm_desc}. Eso fue inaceptable y lo reconozco",
    "{opener}. {harm_verb} {harm_desc}. Fue un error grave",
    "Admito que {harm_verb} {harm_desc}. No hay excusa para eso",
    "Sé que {harm_verb} {harm_desc}. Te hice daño y lo sé",
    "Reconozco que {harm_verb} {harm_desc}. Eso no está bien",
    "{opener}. {harm_desc} y eso te afectó directamente",
    "{harm_verb} {harm_desc}. Fue inaceptable de mi parte",
    "No hay excusa para haber {harm_verb} {harm_desc}",
    "Yo fui quien {harm_verb} {harm_desc}. Me hago responsable",
]

# NEG opening patterns: surface apology + vague reference / external attribution
neg_openings = [
    "Lo siento por la situación",
    "Disculpa por lo que pasó entre nosotros",
    "Perdón si hubo un malentendido",
    "Lo siento, no fue mi intención que las cosas salieran así",
    "Disculpa si lo que dije te molestó",
    "Perdón si no fui claro contigo",
    "Lo siento, solo que las cosas se complicaron",
    "Disculpa por todo esto, pero la situación estaba difícil",
    "Perdón si te sentiste así",
    "Lo siento, es que tenía mucho en mente",
]
...
def build_pos(scenario, harm_verb, harm_desc):
    ...
    opening = random.choice(pos_openings).format(
        opener=opener, harm_verb=harm_verb.capitalize(), harm_desc=harm_desc
    )
...
def build_neg(scenario, harm_verb, harm_desc):
    opening = random.choice(neg_openings)
    ending = random.choice(shared_endings)
    return f"{opening}, {ending}."
